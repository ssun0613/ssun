...
class single_mat():
    ...
    def __getitem__(self, index):
        ...
        data_zero[np.where(image_one.squeeze(axis=0) == 0)] = (116, 116, 116)
        data_zero[np.where(image_one.squeeze(axis=0) == 1)] = (0, 216, 255)
        data_zero[np.where(image_one.squeeze(axis=0) == 2)] = (1, 0, 255)
        image_one = data_zero # image를 RGB형태로 변환하기 위한 code --> training에는 필요없음, wandb에서 image출력하기위해서는 필요


        label = torch.tensor(label_one)
        label = label.squeeze(dim=0)
        # image_2 = Image.fromarray(image_one.astype(dtype=np.uint8)).convert("RGB")
...
if __name__ == '__main__': # class 내부에서 바로 디버깅을 할수있음
    ...
    out = dataset_object_train.__getitem__(135694)
