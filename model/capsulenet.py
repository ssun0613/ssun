...
class Digitcaps(nn.Module):
    ...
    def forward(self, x): # x.shape([1, 1152, 72])
        ...
        x = torch.stack([x]*self.num_capsules, dim=2).unsqueeze(3) # x.shape([1, 1152, 10, 72, 1])

        W = torch.cat([self.W]*batch_size, dim=0) # W.shape([1, 1152, 10, 16, 8])
...
class Decoder(nn.Module):
    ...
    def forward(self, x, data):
        ...
        masked = masked.index_selext(dim=0, index=Variable(max_length_indices.squeeze(1).data))
...
class capsnet(nn.Module):
    def __init__(self):
        ...
        self.digit_capsules = Digitcaps(num_capsules = 10, num_routes=32*6*6, in_channels=8, out_channels=16)
