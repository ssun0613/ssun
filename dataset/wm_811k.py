...
class WM_811K():
    def __init__(self, dataset_path, data_size, is_training=True):
        ...
        self.augmentation = self._toTensor_augmentation(self.is_training)
    ...
    def __getitem__(self, index):
        ...
        if torch.argmax(label) == torch.tensor(0): # data의 balance를 맞추기위해 augmentation하기 위해 조건문 선언
            image = self._toTensor(Image.fromarray(image_one.astype(dtype=np.uint8)).convert("RGB"))

        elif torch.argmax(label) == torch.tensor(2):
            image = self._toTensor(Image.fromarray(image_one.astype(dtype=np.uint8)).convert("RGB"))

        elif torch.argmax(label) == torch.tensor(3):
            image = self._toTensor(Image.fromarray(image_one.astype(dtype=np.uint8)).convert("RGB"))

        elif torch.argmax(label) == torch.tensor(4):
            image = self._toTensor(Image.fromarray(image_one.astype(dtype=np.uint8)).convert("RGB"))

        else:
            image = self.augmentation(Image.fromarray(image_one.astype(dtype=np.uint8)).convert("RGB"))

        return {'image': image, 'label': label}

    def _toTensor(self):
        toTensor = transforms.Compose([transforms.Resize(self.image_size),
                                       transforms.ToTensor()])
        return toTensor

    def _toTensor_augmentation(self, is_training=True):
        if is_training:
           augmentation = transforms.Compose([
                transforms.Resize(self.image_size),
                transforms.RandomHorizontalFlip(),
                transforms.RandomRotation((-90, 90)),
                transforms.RandomResizedCrop((int(self.image_size[0]*0.9),int(self.image_size[1]*0.9))),
                transforms.Resize(self.image_size),
                transforms.ToTensor()])
        else:
            augmentation = None
