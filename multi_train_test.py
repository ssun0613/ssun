...
from options.draw_chart import draw_chart, draw_chart_update
...
def setup_network(opt, device):
    ...
    if not config.opt.continue_train:
        net.init_weights() # network를 초기화하는부분 --> 임의로

    else:
        net = net.load_networks(net=net, net_type=opt.weight_name, weight_path=opt.save_path, device=device)

    return net

def calc_loss(opt, net, input_label, input_image=None):
    if 'googlenet' in opt.network_name:
        from model.loss import multiloss
        output, out_aux1, out_aux2 = net.get_outputs()
        fn_loss_1 = multiloss(input_label, out_aux1)
        fn_loss_2 = multiloss(input_label, out_aux2)
        fn_loss_3 = multiloss(input_label, output)
        fn_loss = fn_loss_1 + fn_loss_2 + fn_loss_3

    elif 'GAIN' in opt.network_name:
        from model.loss import multiloss
...
def setup_dataset(opt):
    if opt.dataset_name in ['wm_811k', 'wm_811k_limited']:
        ...
        test_loader = torch.utils.data.DataLoader(dataset_object_test,
                                                  batch_size=config.opt.batch_size,
                                                  shuffle=True, num_workers=0)
    elif opt.dataset_name in ['data52only']:
...
if __name__ == '__main__':
    ...
    for curr_epoch in range(config.opt.epochs):
        ...
        for batch_id, data in enumerate(train_loader, 1):
            ...
            chart = draw_chart(input_label, prediction)
            chart_update += chart
            ...
            # if global_step % 200 == 0:
                map = draw_chart_update(chart_update)
                if config.opt.show_cam != None:
                    if 'GAIN' in config.opt.network_name:
                        ...
                        wb_logger.log_gain_Grad_result(input_image_pick, GradCAM, input_label_pick, prediction_pick, mode='GAIN_GradCAM' + "_" + str(config.opt.dataset_name) + "{}".format(global_step))
                        print("check wandb\n")

                    else:
                        from model.CAM import GradCAM
                        GradCAM, input_image_pick, input_label_pick, prediction_pick = GradCAM(net, input_label)
                        wb_logger.log_cam_to_wandb(input_image_pick, GradCAM, input_label_pick, prediction_pick, mode='train_GradCAM' + "_" + str(config.opt.dataset_name)+ "{}".format(global_step))
                        print("check wandb\n")
            ...
            if global_step % config.opt.freq_save_net == 0:
                torch.save({'net': net.state_dict()}, config.opt.save_path + '/{}_epoch'.format(config.opt.network_name) + ".pth")
